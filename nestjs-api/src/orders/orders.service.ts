...
import { Order, OrderDocument, OrderSchema, OrderStatus, OrderType } from './entities/order.entity';
...
export class OrdersService implements OnModuleInit {
  ...
  async createTrade(dto: CreateTradeDto) {
    ...
    try {
      ...
      const order = (
        await this.orderSchema
        .findById(dto.orderId)
        .session(session)
      ) as OrderDocument & { trades: string[] };
      ...
      if (dto.status === OrderStatus.CLOSED) {
        ...
        if (walletAsset) {
          ...
          const wallet = (
            await this.walletSchema.findById(order.wallet)
